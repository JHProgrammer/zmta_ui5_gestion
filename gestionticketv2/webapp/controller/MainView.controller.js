sap.ui.define([
   ...
   */
  function (Controller, constantes, service, Label, Popover, coreLibrary, DateFormat, Fragment, JSONModel, Log, UI5Date) {
    "use strict";

    var ValueState = coreLibrary.ValueState;

    return Controller.extend("cap.gestionticketv2.controller.MainView", {
      onInit: function () {
      ...
      onGetConsultores: async function () {
        var self = this;
        let localmodel = self.getView().getModel("localModel");
        try {
          //   const oParam = enviarDatos.obtenerRolesXGestionticket(idGestionticket);
          ...
          if (response) {
            localmodel.setProperty("/aConsultores", response);
            // const cliente = response.results;
            // console.log("Cliente",cliente);
            // const listarRolesXGestionticket =
            //   recibirDatos.obtenerRolesXGestionticket(roles);
      ...
      onGetLideres: async function () {
        var self = this;
        let localmodel = self.getView().getModel("localModel");
        try {
          //   const oParam = enviarDatos.obtenerRolesXGestionticket(idGestionticket);
          let response = await service.serviceGet(
            constantes.services.obtenerLider,
            [],
            self
          );
          if (response) {
            localmodel.setProperty("/aLideres", response);
            // const cliente = response.results;
            // console.log("Cliente",cliente);
            // const listarRolesXGestionticket =
            //   recibirDatos.obtenerRolesXGestionticket(roles);
      ...
      _arrangeDialogFragment: function (iDialogType) {
        ...
        if (!this._pNewAppointmentDialog) {
          this._pNewAppointmentDialog = Fragment.load({
            id: oView.getId(),
            name: "cap.gestionticketv2.fragment.Create",
            controller: this
      ...
      _handleSingleAppointment: function (oAppointment) {
        ...
        if (!this._pDetailsPopover) {
          this._pDetailsPopover = Fragment.load({
            id: oView.getId(),
            name: "cap.gestionticketv2.fragment.Details",
            controller: this
